...
(async () => {
  ...
  try {
    ...
    const sidebarLink=page.locator('.wiki-sidebar a[href="/l4d_api_reference.html"]');
    ...
    assert.equal(new URL(api.url()).pathname,'/l4d_api_reference.html');
    ...
    await api.goto(base+'/l4d_api_reference.html',{waitUntil:'networkidle'});
    ...
    for(const route of ['/products/l4d/','/guides/l4d/','/guides/l4d/menu-settings/']) {
      ...
      const attrs=await page.locator('a[href="/l4d_api_reference.html"]').evaluateAll(links=>links.map(a=>({target:a.target,rel:a.rel})));
    ...
    await page.locator('#guide-results a[href^="/l4d_api_reference.html"]').first().waitFor();
