...
(async()=>{
 ...
 for(const width of [1440,375,320]){
  ...
  for(const slug of ['half-life-2','half-life-2-deathmatch']){
   ...
   assert.equal(await page.locator('.product-status.is-released').innerText(),'Released');
   assert.equal(await page.locator('.product-status.is-free').innerText(),'Free');
   const text=await page.locator('main').innerText();
   assert(text.includes('FREE ACCESS') && text.includes('Compatibility:') && text.includes('Start game'));
   if(slug==='half-life-2-deathmatch')assert(text.includes('-insecure') && text.includes('x86 / x64'));
   else assert(text.includes('Episodes are not included.'));
   await page.locator('.hl2-artwork img').evaluate(image=>image.decode());
   assert(!await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),'Released page overflow: '+slug+' '+width);
   const art=await page.locator('.hl2-artwork').boundingBox(),details=await page.locator('.purchase-card').boundingBox();
   assert(width>1100?art.x+art.width<=details.x:art.y+art.height<=details.y,'Artwork overlaps access card: '+slug+' '+width);
   assert.equal(await page.getByRole('link',{name:'Get a free key',exact:true}).getAttribute('href'),'/scoobyontop.html');
   for(const [label,target] of [['Product details','product-details'],['Setup guide','setup-guide']]){
    await page.getByRole('link',{name:label,exact:true}).click();
    assert.equal(new URL(page.url()).hash,'#'+target);
    assert(await page.locator('#'+target).evaluate(e=>{const r=e.getBoundingClientRect();return r.top>=80&&r.top<innerHeight}),'Section obscured after navigation: '+target);
   }
   await page.evaluate(()=>{document.activeElement.blur();scrollTo(0,0)});
   if(width!==320)await page.screenshot({path:path.join(output,'released-'+slug+'-'+width+'.jpg'),type:'jpeg',quality:80,fullPage:true});
  }
 }
